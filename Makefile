cc := clang++
cflags := -std=c++20 -g -Wall -Wextra -Werror
driver_t := driver
driver_s := driver.cpp ipc_cpp/mem.cpp program.cpp
driver_o := $(patsubst %.cpp,%.o,$(driver_s))
driver_d := $(patsubst %.cpp,%.d,$(driver_s))
driver_c := $(cflags)
driver_l := -lboost_system -lboost_thread
driver_gfx_t := driver_gfx
driver_gfx_s := driver_gfx.cpp ipc_cpp/mem.cpp program.cpp
driver_gfx_o := $(patsubst %.cpp,%.o,$(driver_gfx_s))
driver_gfx_d := $(patsubst %.cpp,%.d,$(driver_gfx_s))
driver_gfx_c := $(cflags)
driver_gfx_l := libraylib.a -lm -lboost_system -lboost_thread
...
clean:
	...
	- rm $(driver_gfx_d) $(driver_gfx_o) $(driver_gfx_t) \
