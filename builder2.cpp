...
struct Module {
    ...
    std::vector<std::string> apt_packages; // MODULE_FETCH_APT_PACKAGES
...
// apt cache: packages we've already installed (persisted across runs)
static std::unordered_set<std::string> g_installed_apt_packages;
static bool g_apt_updated = false; // whether we've already run "apt-get update" in this run

static Module &get_module(const std::string &name);

// Forward declarations for apt cache helpers
static void load_apt_cache();
static void save_apt_cache();
static void ensure_apt_packages(Module &m);
...
static void parse_build_module(Module &m) {
    ...
    while (std::getline(in, line)) {
        ...
        if (key == "MODULE_NAME") {
            ...
            m.fetch_url = val;
        } else if (key == "MODULE_FETCH_APT_PACKAGES") {
            m.apt_packages = split_ws(val);
        }
    }
...
static fs::path apt_cache_file_path() {
    return g_cache_root / "apt_installed.txt";
}

static void load_apt_cache() {
    fs::create_directories(g_cache_root);
    fs::path cache_file = apt_cache_file_path();
    ...
    while (std::getline(in, line)) {
        line = trim(line);
        if (!line.empty()) {
            g_installed_apt_packages.insert(line);
        }
    }
}
...
static void save_apt_cache() {
    ...
    if (!out) {
        std::cerr << "[warn] failed to write apt cache file: " << cache_file << "\n";
        return;
    }
    for (const auto &pkg : g_installed_apt_packages) {
        out << pkg << "\n";
    }
}

// Install MODULE_FETCH_APT_PACKAGES for a module, with caching.
static void ensure_apt_packages(Module &m) {
    // Collect packages that are not yet known to be installed.
    std::vector<std::string> to_install;
    for (const auto &pkg : m.apt_packages) {
        if (pkg.empty()) continue;
        if (g_installed_apt_packages.find(pkg) == g_installed_apt_packages.end()) {
    ...
    for (const auto &pkg : to_install) {
        g_installed_apt_packages.insert(pkg);
    }
}
...
static void compile_module(const std::string &name) {
    ...
    // Ensure any apt packages required by this module are installed (once per package)
    ensure_apt_packages(m);
...
int main(int argc, char **argv) {
    ...
    // Load apt-installed cache from previous runs
    load_apt_cache();
