...
path_t zip_t::zip(const path_t& dir, const path_t& install_zip_path) {
    if (install_zip_path.extension() != ".zip") {
        throw std::runtime_error(std::format("zip_t::zip: install path '{}' must have .zip extension", install_zip_path));
    }

    if (filesystem_t::exists(install_zip_path)) {
        throw std::runtime_error(std::format("zip_t::zip: install path '{}' already exists", install_zip_path));
    }
    ...
    if (!mz_zip_writer_init_file(&zip, install_zip_path.string().c_str(), 0)) {
        throw std::runtime_error(std::format("zip_t::zip: failed to create zip file at '{}'", install_zip_path));
    }

    for (const auto& regular_file : regular_files) {
        if (!filesystem_t::exists(regular_file)) {
            mz_zip_writer_end(&zip);
            throw std::runtime_error(std::format("zip_t::zip: file '{}' does not exist", regular_file));
        }
        if (!filesystem_t::is_regular_file(regular_file)) {
            mz_zip_writer_end(&zip);
            throw std::runtime_error(std::format("zip_t::zip: file '{}' is not a regular regular_file", regular_file));
        }
        ...
        if (!mz_zip_writer_add_file(&zip, rel.c_str(), regular_file.c_str(), nullptr, 0, MZ_DEFAULT_COMPRESSION)) {
            mz_zip_writer_end(&zip);
            throw std::runtime_error(std::format("zip_t::zip: failed to add file '{}' to zip archive", regular_file));
        }
    }

    if (!mz_zip_writer_finalize_archive(&zip)) {
        mz_zip_writer_end(&zip);
        throw std::runtime_error(std::format("zip_t::zip: failed to finalize zip archive at '{}'", install_zip_path));
    }
...
path_t zip_t::unzip(const path_t& zip_path, const path_t& install_dir) {
    if (zip_path.extension() != ".zip") {
        throw std::runtime_error(std::format("zip_t::unzip: zip file '{}' must have .zip extension", zip_path));
    }

    if (filesystem_t::exists(install_dir) && !filesystem_t::is_directory(install_dir)) {
        throw std::runtime_error(std::format("zip_t::unzip: install path '{}' exists and is not a directory", install_dir));
    }

    if (!filesystem_t::exists(zip_path) || !filesystem_t::is_regular_file(zip_path)) {
        throw std::runtime_error(std::format("zip_t::unzip: zip file '{}' does not exist or is not a regular file", zip_path));
    }
    ...
    if (!mz_zip_reader_init_file(&zip, zip_path.string().c_str(), 0)) {
        throw std::runtime_error(std::format("zip_t::unzip: failed to open zip file at '{}'", zip_path));
    }
    ...
    for (mz_uint i = 0; i < num_files; ++i) {
        ...
        if (!mz_zip_reader_extract_to_file(&zip, i, out_path.string().c_str(), 0)) {
            ...
            throw std::runtime_error(std::format("zip_t::unzip: failed to extract file '{}' to '{}'", st.m_filename, out_path));
        }
    }
