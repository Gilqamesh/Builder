...
void configure(const filesystem::path_t& source_dir, const filesystem::path_t& build_dir, const std::vector<std::pair<std::string, std::string>>& define_key_values) {
    ...
    std::vector<process::process_arg_t> configure_process_args;
    ...
    const int configure_process_result = process::create_and_wait(configure_process_args);
    if (0 < configure_process_result) {
        throw std::runtime_error(std::format("cmake::configure: cmake configure command failed with exit code {}", configure_process_result));
    } else if (configure_process_result < 0) {
        throw std::runtime_error(std::format("cmake::configure: cmake configure command terminated by signal {}", -configure_process_result));
    }
}
...
void build(const filesystem::path_t& build_dir, std::optional<size_t> n_jobs) {
    ...
    std::vector<process::process_arg_t> build_process_args;
    ...
    const int build_process_result = process::create_and_wait(build_process_args);
    if (0 < build_process_result) {
        throw std::runtime_error(std::format("cmake::build: cmake build command failed with exit code {}", build_process_result));
    } else if (build_process_result < 0) {
        throw std::runtime_error(std::format("cmake::build: cmake build command terminated by signal {}", -build_process_result));
    }
}

void install(const filesystem::path_t& build_dir) {
    if (!filesystem::exists(build_dir)) {
        throw std::runtime_error(std::format("cmake::install: build_dir '{}' does not exist", build_dir));
    }

    const int install_process_result = process::create_and_wait({
        CMAKE_PATH,
        "--install",
        build_dir
    });
    if (0 < install_process_result) {
        throw std::runtime_error(std::format("cmake::install: cmake install command failed with exit code {}", install_process_result));
    } else if (install_process_result < 0) {
        throw std::runtime_error(std::format("cmake::install: cmake install command terminated by signal {}", -install_process_result));
    }
}
