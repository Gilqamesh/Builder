...
static void relaunch_newer_version(const std::filesystem::path& modules_dir, const std::filesystem::path& artifacts_dir, const std::string& target_module, uint64_t new_version) {
    ...
    //     throw module_exception_t(module, "failed to compile temporary next version of builder binary '{}'", tmp_binary_path_str));
    // }

    // if (!std::filesystem::exists(tmp_binary_path)) {
    //     unlink(tmp_binary_path_str.c_str());
    //     throw module_exception_t(module, "temporary next version of builder binary '{}' was not created, something went wrong", tmp_binary_path_str));
    // }
    ...
    // unlink(tmp_binary_path_str.c_str());
    // if (fd == -1) {
    //     throw module_exception_t(module, std::format("failed to open temporary next version of builder binary '{}', errno: {}, errno_msg: {}", tmp_binary_path_str, errno_val, std::strerror(errno_val)));
    // }
    // extern char **environ;
    // fexecve(fd, argv.data(), environ);

    // close(fd);
    // throw module_exception_t(module, std::format("fexecve failed to run temporary next version of builder binary '{}', errno: {}, errno_msg: {}", tmp_binary_path_str, errno, std::strerror(errno)));
}
...
//     } else {
//         // at this point we need bundled static pic libraries in linking order (reverse topo order, i.e., most dependent first
//         // to have unresolved symbols early for the linker
//         // and shared libraries in any order
//         // [bundled static libraries, shared libraries]
...
//         // to build the builder_plugin, we need to call each dependency builder_plugin dependency module of the builder_plugin in a topological order, and create a  builder__export_bundle_position_independent
...
//             throw module_exception_t(module, std::format("builder plugin '{}' was not created, something went wrong", builder_plugin_so.string()));
//         }
...
//             throw module_exception_t(module, message);
//         }
...
//             throw module_exception_t(module, std::format("failed to create SCC artifact directory '{}'", scc_version_dir.string()));
//         }
...
//                         throw module_exception_t(module, std::format("expected archive '{}' to exist for module '{}'", archive.string(), module->name));
//                     }
...
//     if (!std::filesystem::exists(module.artifact_dir)) {
//         throw module_exception_t(module, std::format("artifact directory '{}' does not exist for module '{}'", module.artifact_dir.string(), module.name));
//     }
...
//             if (!std::filesystem::exists(builder_plugin_so)) {
//                 throw module_exception_t(module, std::format("builder plugin '{}' does not exist", builder_plugin_so.string()));
//             }
