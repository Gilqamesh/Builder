...
path_t gzip_t::gzip(const path_t& file, const path_t& install_gzip_path) {
    if (install_gzip_path.extension() != ".gz") {
        throw std::runtime_error(std::format("gzip_t::gzip: install path '{}' must have .gz extension", install_gzip_path));
    }

    if (filesystem_t::exists(install_gzip_path)) {
        throw std::runtime_error(std::format("gzip_t::gzip: install path '{}' already exists", install_gzip_path));
    }

    if (!filesystem_t::exists(file) || !filesystem_t::is_regular_file(file)) {
        throw std::runtime_error(std::format("gzip_t::gzip: input '{}' is not a regular file", file));
    }
    ...
    if (!ifs) {
        throw std::runtime_error(std::format("gzip_t::gzip: failed to open input '{}'", file));
    }

    gzFile gzip = gzopen(install_gzip_path.string().c_str(), "wb");
    if (!gzip) {
        throw std::runtime_error(std::format("gzip_t::gzip: failed to open output '{}'", install_gzip_path));
    }
    ...
    while (1) {
        ...
        if (ifs.fail() || ifs.bad()) {
            gzclose(gzip);
            throw std::runtime_error(std::format("gzip_t::gzip: read failed from input '{}'", file));
        }
    }

    if (gzclose(gzip) != Z_OK) {
        throw std::runtime_error(std::format("gzip_t::gzip: failed to close output '{}'", install_gzip_path));
    }
...
path_t gzip_t::ungzip(const path_t& gzip_path, const path_t& install_file) {
    if (gzip_path.extension() != ".gz") {
        throw std::runtime_error(std::format("gzip_t::ungzip: gzip path '{}' must have .gz extension", gzip_path));
    }

    if (!filesystem_t::exists(gzip_path) || !filesystem_t::is_regular_file(gzip_path)) {
        throw std::runtime_error(std::format("gzip_t::ungzip: input '{}' is not a regular file", gzip_path));
    }

    if (filesystem_t::exists(install_file)) {
        throw std::runtime_error(std::format("gzip_t::ungzip: install file '{}' already exists", install_file));
    }
    ...
    if (!ofs) {
        throw std::runtime_error(std::format("gzip_t::ungzip: failed to open output '{}'", install_file));
    }

    gzFile gz = gzopen(gzip_path.string().c_str(), "rb");
    if (!gz) {
        throw std::runtime_error(std::format("gzip_t::ungzip: failed to open input '{}'", gzip_path));
    }
    ...
    while (1) {
        ...
        if (!ofs) {
            gzclose(gz);
            throw std::runtime_error(std::format("gzip_t::ungzip: write failed to output '{}'", install_file));
        }
    }

    if (gzclose(gz) != Z_OK) {
        throw std::runtime_error(std::format("gzip_t::ungzip: failed to close input '{}'", gzip_path));
    }
