...
reader_t::reader_t()
{
  ...
  set_macro_character('"', [](reader_t& reader, istream& is, char c) {
    reader.push(c);
...
expr_t* reader_t::read(istream& is, bool recursive) {
  string lexeme;
...
static expr_t* read(string& lexeme, istream& is, bool recursive) {
  if (is_at_end()) {
    if (eof_error_p) {
      throw runtime_error("");
    } else {
...
static expr_t* step8(char c, bool recursive) {
  if (is_at_end()) {
    return step10(c, recursive);
  }
  char y = eat();
  if (is_constituent(y) || is_non_terminating_macro(y)) {
    push(y);
    return step8(y, recursive);
    ...
    char z = eat();
    push(z);
    return step8(z, recursive);
  } else if (is_multiple_escape(y)) {
    return step9(y, recursive);
...
static expr_t* step9(char c, bool recursive) {
  if (is_at_end()) {
...
expr_t* step10(char c, bool recursive) {
  return (expr_t*) new expr_symbol_t(lexeme);
}

expr_t* read_illegal(char c, bool recursive) {
  throw runtime_error("illegal character");
}

static expr_t* read_whitespace(istream& is, char c, bool recursive) {
  if (recursive) {
    lexeme.push_back(c);
  }
  return read(is, recursive);
}

expr_t* read_terminating_macro(string& lexeme, reader_t& reader, char c, bool recursive) {
  expr_t* result = get_macro_character(c)(*this, c);
  if (result) {
    return result;
  }
  return read(recursive);
}

expr_t* reader_t::read_non_terminating_macro(char c, bool recursive) {
  expr_t* result = get_macro_character(c)(*this, c);
  if (result) {
    return result;
  }
...
expr_t* reader_t::read_single_escape(char c, bool recursive) {
  if (is_at_end()) {
...
expr_t* reader_t::read_multiple_escape(char c, bool recursive) {
  clear();
  return step9(c, recursive);
}

expr_t* reader_t::read_constituent(char c, bool recursive) {
  clear();
  push(c);
  return step8(c, recursive);
}
